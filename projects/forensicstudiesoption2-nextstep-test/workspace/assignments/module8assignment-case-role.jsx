...
export function getCaseGrade(mistakes) {
  if (mistakes > 2) return { label: "COMPROMISED", tone: "#e11d48" };
  if (mistakes > 0) return { label: "ACCEPTABLE", tone: "#f59e0b" };
  return { label: "EXEMPLARY", tone: "#10b981" };
}

export default function LIMSOS_Simulation({ onStateChange, initialState }) {
  const [systemState, setSystemState] = useState(() => {
    const next = initialState?.systemState;
    if (next === "active" || next === "summary" || next === "booting") {
      return next;
    }
    return "booting";
  }); // booting, active, summary
  const [currentPhaseIndex, setCurrentPhaseIndex] = useState(() => {
    const next = Number.isInteger(initialState?.currentPhaseIndex) ? initialState.currentPhaseIndex : 0;
    return Math.max(0, Math.min(casePhases.length - 1, next));
  });
  const [logs, setLogs] = useState(Array.isArray(initialState?.logs) ? initialState.logs : []);
  const [selectedChoiceIndex, setSelectedChoiceIndex] = useState(null);
  const [mistakes, setMistakes] = useState(Number(initialState?.mistakes) || 0);
  const [isTransferring, setIsTransferring] = useState(false);
  const [time, setTime] = useState(new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }));
  const [decisionHistory, setDecisionHistory] = useState(
    Array.isArray(initialState?.decisionHistory) ? initialState.decisionHistory : []
  );
  ...
  }, [decisionHistory, logs, mistakes, onStateChange, systemState, currentPhaseIndex]);
  ...
  const handleChoice = (index) => {
    ...
    setDecisionHistory(prev => [
      ...prev,
      {
        department: activePhase.department,
...
function SummaryScreen({ mistakes, reset }) {
  const { label: grade, tone } = getCaseGrade(mistakes);
  const gradeColor = tone === "#10b981" ? "text-emerald-400" : tone === "#f59e0b" ? "text-amber-400" : "text-rose-500";
