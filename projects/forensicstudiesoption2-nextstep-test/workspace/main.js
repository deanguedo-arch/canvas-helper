(function () {
  ...
  function syncChapterProgressFrame(chapterId, focusComponentId = "") {
    ...
    frame.contentWindow.postMessage({
      type: "forensicstudiesoption2-nextstep-test-module-progress-sync",
      chapterId,
  ...
  function hasAuthoredQuizSections(quiz) {
    return !!(
      (quiz?.multipleChoice || []).length
      || (quiz?.trueFalse || []).length
      || (quiz?.writtenResponse || []).length
      || (quiz?.matching?.items || []).length
      || (quiz?.matching?.questions || []).length
      || (quiz?.studentChoice?.options || []).length
    );
  }
  ...
  function computeQuizSectionBreakdown(quiz) {
    ...
    const multipleChoiceSection = {
      ...
      total: completion.multipleChoiceTotal
    };
    ...
    const trueFalseSection = {
      ...
      total: completion.trueFalseTotal
    };
    ...
    const matchingSection = {
      ...
      total: completion.matchingTotal
    };
  ...
  function renderProgress() {
    ...
    refs.courseSubtitle.textContent = data.course?.subtitle || "Module content, assignments, and quizzes are organized in one place.";
  ...
  function retakeQuiz(id) {
    ...
    state.progress.quizComplete[id] = false;
    delete state.progress.quizCompletedAt[id];
  ...
  function renderSectionHeader() {
    ...
    refs.sectionHeader.hidden = hideSectionHeader;
    ...
    if (state.tab === "assignments") {
      ...
      refs.sectionIntro.textContent = "Open a module assignment to review the instructions and complete the embedded workspace.";
      return;
    }

    refs.sectionTitle.textContent = "Home";
    refs.sectionIntro.textContent = "Each module includes lesson pages, assignments, and quizzes from the Forensic Studies course.";
  }
    ...
    return `
      <article class="detail-card chapter-detail-card chapter-detail-surface" style="--accent:${escapeHtml(chapter.accent || "#8b6728")}">
        <div class="detail-stack chapter-detail-layout">
          ...
          <div class="lock-copy chapter-detail-note">Use the connected source materials and assessments while this lesson surface is being built out.</div>
          <div class="detail-actions">
            ${detailActions}
          </div>
        </div>
  ...
  function renderAssignmentDetail(assignment) {
    ...
    const briefSection = briefs.length
      ? `
          <section class="assignment-brief-shell" aria-label="Assignment details">
            <div class="assignment-brief-grid">
              ${briefs.map((brief) => `
                <article class="assignment-brief-card">
                  <span class="detail-eyebrow">Assignment details</span>
                  <h5 class="assignment-brief-title">${escapeHtml(brief.title || "Assignment brief")}</h5>
                  ${brief.instructionHtml ? `<div class="assignment-brief-html">${brief.instructionHtml}</div>` : brief.summary ? `<p class="assignment-brief-copy">${escapeHtml(brief.summary)}</p>` : ""}
                  ${
                    brief.individualized || brief.identified
                      ? `
                        <div class="assignment-evidence-grid">
                          ${brief.individualized ? `
                            <article class="assignment-evidence-card is-individualized">
                              <span class="detail-eyebrow">Individualized evidence</span>
                              <p>${escapeHtml(brief.individualized)}</p>
                            </article>
                          ` : ""}
                          ${brief.identified ? `
                            <article class="assignment-evidence-card is-identified">
                              <span class="detail-eyebrow">Identified evidence</span>
                              <p>${escapeHtml(brief.identified)}</p>
                            </article>
                          ` : ""}
                        </div>
                      `
  ...
  function getQuizSectionIntro(section, complete) {
    if (!complete) {
      return "Generate results to finalize this attempt. After that, you can check answers or retake the quiz.";
    }
  ...
  function renderSourceLinkedQuizDetail(quiz, complete, completedAt) {
    return `
      <article class="quiz-shell quiz-detail-card quiz-detail-surface" style="--accent:${escapeHtml(quiz.accent || "#8b6728")}">
        <div class="quiz-stack quiz-detail-layout">
          <div class="quiz-header">
            <div class="quiz-copy">
              <p class="detail-eyebrow">${escapeHtml(data.course?.title || "Course")} · Assessment</p>
              <h4 class="quiz-page-title">${escapeHtml(`${quiz.code}: ${quiz.title}`)}</h4>
              <p class="detail-summary">${escapeHtml(quiz.summary || "This quiz is linked directly to the original exported source.")}</p>
            </div>
            <div class="quiz-meta-row">
              <div class="quiz-meta-block">
                <span class="metric-label">Status</span>
                <strong>${complete ? "Marked complete" : "Source-linked"}</strong>
              </div>
              <div class="quiz-meta-block">
          ...
          <div class="quiz-evaluation-panel">
            <div class="quiz-evaluation-copy">
              <h5>Source-linked assessment</h5>
              <p>This assessment opens from the retained course file because no authored question set was available for it in this build.</p>
            </div>
            <div class="quiz-evaluation-score">
              <strong><span class="quiz-evaluation-earned">Source</span><small>/linked</small></strong>
              <span class="quiz-evaluation-status">Open the original exported file</span>
            </div>
          </div>

          <div class="quiz-actions quiz-actions-row">
            <button class="btn btn-secondary" type="button" data-generate-quiz-results="${escapeHtml(quiz.id)}">Generate Results</button>
            <button class="btn btn-muted" type="button" data-retake-quiz="${escapeHtml(quiz.id)}" ${complete ? "" : "disabled"}>Retake Quiz</button>
            <button class="btn quiz-back-link" type="button" data-back-home="quizzes">Back to quizzes <span aria-hidden="true">→</span></button>
          </div>

          <div class="lock-copy">When this quiz is rebuilt later, the interactive sections can be dropped into the same shell without changing the surrounding navigation.</div>
        </div>
      </article>
    `;
  }
  ...
  function renderQuizDetail(quiz) {
    ...
    if (!hasAuthoredQuizSections(quiz) && quiz.sourcePath) {
      return renderSourceLinkedQuizDetail(quiz, complete, completedAt);
    }
    ...
    const section = sections.find((item) => item.key === state.quizSection)?.key || sections[0]?.key || "mc";
    ...
    return `
      <article class="quiz-shell quiz-detail-card quiz-detail-surface" style="--accent:${escapeHtml(quiz.accent || "#8b6728")}">
        <div class="quiz-stack quiz-detail-layout">
          ...
          <div class="quiz-actions quiz-actions-row">
            <button class="btn btn-primary" type="button" data-generate-quiz-results="${escapeHtml(quiz.id)}">Generate Results</button>
...
<body>
  ...
  function generateQuizResults(quiz) {
    if (!quiz) return;
    state.progress.quizComplete[quiz.id] = true;
    state.progress.quizCompletedAt[quiz.id] = new Date().toISOString();
    saveProgress();
  ...
  window.addEventListener("message", (event) => {
    ...
    if (payload.type === "forensicstudiesoption2-nextstep-test-module-progress-ready" && typeof payload.chapterId === "string") {
      syncChapterProgressFrame(payload.chapterId);
      return;
    }

    if (
      payload.type === "forensicstudiesoption2-nextstep-test-module-progress-update"
