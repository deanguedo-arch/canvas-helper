...
- Current learner SHA: `21363490e611251c01101cee0bc5925583d1b856b8285a87ee74838ddd1341e5`; tree SHA: `e9f39a3171ec6365fdd90863a268b0383e065d95efd64b1915d410a57f02c59a`.
...
- Start at [the canonical playbook](../../biology30-unit-a-pilot/meta/unit-a-to-bcd-improvement-playbook.md#current-production-recipe), then [the source-grounded remaining academic review](./remaining-academic-review.md), [the earlier structural evidence inventory](./final-academic-review.json), [both-pilot transfer contract](../../biology30-unit-a-pilot/meta/biology30-improvement-transfer-contract.json), and [B-D material readiness](../../biology30-unit-a-pilot/meta/bcd-material-readiness.json).
...
- The current Chapter 11 batch adds six saved diagram/reasoning tasks inside Models and Data Lab and repairs the numeric action-potential graph. See [the authored task contract](./chapter-11-academic-repair.json). Existing question IDs, core prose, counts, state schema and completion rules remain unchanged; six new allowlisted response IDs raise the ordinary-text worst-case estimate to 42,284 characters. Only 1,716 characters remain under the target. Practical skills and later-chapter repairs remain open.
- The subsequent review-only pass has checked all 86 practice items, all 36 overlapping gaps, the standards source columns and all 14 local media paths. See [the machine review](./remaining-academic-review.json). It records 12 invalid Final Practice chapter/page links, 22 unrelated printed-page selections, 68 generic feedback records and first-position answer bias. All local media paths are still text summaries; one actual publisher transcript was reviewed, thirteen remain pending. Availability and English-track metadata are not transcript approval.
- Do not repeat the earlier claim of 53 official acceptable behaviours. The stored rows comprise 47 acceptable examples, five excellence examples and one local outcome-derived criterion. Performance examples are illustrative; use the Program of Studies for required content. The new report corrects the old gap inventory's scope claims without deleting history or approving the course.
- Review evidence is exact-build and read-only: `npx tsx scripts/review-biology30-unit-a-pilot-2-academic.ts --check`. Focused tests: `npx tsx --test scripts/tests/biology30-unit-a-academic-review.test.ts`. No learner rebuild, E2E rerun or deployment occurred for these report-only changes. The next repair batch needs explicit authorization; no transcript review or practical delivery decision is silently marked complete.
