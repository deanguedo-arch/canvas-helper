(function () {
  const STORAGE_KEY = "worldreligions30-option1.assignment.chapter2interactive";
  const HEIGHT_MESSAGE = "wr30-assignment-height";
  const FRAME_KEY = "chapter2interactive";
  const STEPS = [
    {
      id: "identification",
      title: "Artifact Identification",
      marks: 2,
      copy: "Identify the teaching, symbol, or object and name the nation or community connected to it when that information is available."
    },
    {
      id: "description",
      title: "Description & Usage",
      marks: 4,
      copy: "Describe what the teaching, symbol, or object means and how it is used."
    },
    {
      id: "values",
      title: "Values & Beliefs",
      marks: 4,
      copy: "Explain the values, beliefs, or worldview ideas communicated through the teaching, symbol, or object."
    },
    {
      id: "practice",
      title: "Respectful Practice",
      marks: 3,
      copy: "Explain one respectful practice a non-Indigenous student should follow when learning about it."
    },
    {
      id: "bibliography",
      title: "Bibliography",
      marks: 3,
      copy: "Record at least two sources and prioritize Indigenous-authored or community-grounded material when possible."
    },
    {
      id: "report",
      title: "Final Report",
      marks: 16,
      copy: "Review the compiled report, make sure the language stays nation-specific and respectful, then generate the printable version when it is ready."
    }
  ];
  ...
  function getCompletedStepCount() {
    ...
    return [
      Boolean(f.itemName && f.nation),
      Boolean(f.description),
      Boolean(f.values),
      Boolean(f.practice),
      Boolean(f.source1 && f.source2),
      Boolean(f.itemName && f.description && f.values)
  ...
  function getReportData() {
    const f = state.fields;
    const checklist = [
      { label: "Teaching, symbol, or object recorded", complete: Boolean(f.itemName) },
      { label: "Nation or community recorded", complete: Boolean(f.nation) },
      { label: "Description and usage explained", complete: Boolean(f.description) },
      { label: "Values or beliefs explained", complete: Boolean(f.values) },
      { label: "Respectful practice note added", complete: Boolean(f.practice) },
    ...
    return {
      ready,
      title: f.itemName || "Unnamed teaching or symbol",
      subtitle: f.nation ? `Nation or community: ${f.nation}` : "Nation or community not yet recorded.",
      intro: "A respectful report built from the research notes below. Confirm the nation-specific language, then generate the printable version from this page.",
      meta: [
        { label: "Nation or community", value: f.nation || "Not yet recorded" },
        { label: "Respectful practice", value: f.practice || "Not yet recorded" },
      ...
      sections: [
        {
          ...
          value: f.description || "No description recorded yet.",
          wide: true
        },
        {
          label: "Values & beliefs",
          value: f.values || "No values or beliefs section recorded yet."
        },
        {
          label: "Respectful practice",
          value: f.practice || "No respectful practice note recorded yet."
        },
        {
          label: "Bibliography",
          list: [
  ...
  function renderHeader() {
    ...
    refs.stepCode.textContent = `Chapter 2 · Step ${state.activeStep + 1}`;
    ...
    return `
      <div class="step-section">
        <div class="field-grid">
          ${renderField("Teaching, symbol, or object", "itemName", "Example: Medicine Wheel, Sweetgrass, Wampum")}
          ${renderField("Nation or community", "nation", "Example: Anishinaabe, Haudenosaunee, Inuit")}
        </div>
      </div>
    `;
  }
    ...
    return `
      <div class="step-section">
        ${renderTextArea(
          ...
          "description",
          "Describe what it means and how it is used in sufficient detail.",
          "You can include context, ceremonial use, teaching value, or where it appears."
        )}
      </div>
    `;
  }
    ...
    return `
      <div class="step-section">
        ${renderTextArea(
          "Values and beliefs",
          "values",
          "Explain what values or beliefs this teaching, symbol, or object communicates.",
          "You might discuss interconnection, responsibility, respect, balance, memory, or relationship to land."
        )}
      </div>
    `;
  }
    ...
    return `
      <div class="step-section">
        ${renderTextArea(
          "Respectful practice",
          "practice",
          "Explain one respectful practice a non-Indigenous student should follow when learning about it.",
          "You can address protocol, nation specificity, humility, source selection, or avoiding appropriation."
        )}
      </div>
    `;
  }
...
<body>
  <div class="report-page">
    ...
    <header class="report-head">
      <div>
        <p class="report-code">Chapter 2 Report</p>
  ...
  function renderPanel() {
    switch (STEPS[state.activeStep].id) {
      ...
      case "values":
        ...
        return;
      case "practice":
        refs.panel.innerHTML = renderPractice();
        return;
      case "bibliography":
