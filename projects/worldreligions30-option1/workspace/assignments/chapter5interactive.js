(function () {
  const STORAGE_KEY = "worldreligions30-option1.assignment.chapter5interactive";
  const HEIGHT_MESSAGE = "wr30-assignment-height";
  const FRAME_KEY = "chapter5interactive";
  const STEPS = [
    {
      id: "identification",
      title: "Artifact Identification",
      marks: 2,
      copy: "Identify the Buddhist symbol, artwork, monument, or meditation object you selected before you begin the explanation."
    },
    {
      id: "description",
    ...
    {
      id: "values",
      title: "Teachings and Values",
      marks: 4,
      copy: "Explain what Buddhist teachings or values it communicates."
    },
    {
      id: "interpretation",
      title: "Respectful Interpretation",
      marks: 3,
      copy: "Describe one reason a respectful interpretation matters."
    },
    {
      id: "bibliography",
      title: "Bibliography",
  ...
  function getCompletedStepCount() {
    ...
    return [
      ...
      Boolean(f.values),
      Boolean(f.interpretation),
      Boolean(f.source1 && f.source2),
      Boolean(f.itemName && f.description && f.values && f.interpretation)
  ...
  function getReportData() {
    const f = state.fields;
    const checklist = [
      { label: "Selected symbol or artwork recorded", complete: Boolean(f.itemName) },
      { label: "Detailed description recorded", complete: Boolean(f.description) },
      { label: "Teachings or values explained", complete: Boolean(f.values) },
      { label: "Respectful interpretation explained", complete: Boolean(f.interpretation) },
    ...
    return {
      ready,
      title: f.itemName || "Unnamed Buddhist symbol or artwork",
      subtitle: f.itemName ? `Buddhist symbol or artwork study: ${f.itemName}` : "Select a Buddhist symbol or artwork to build the report.",
      intro: "A Chapter 5 study report built from the notes below. Review the wording, then generate the printable version from this page.",
      meta: [
        { label: "Selected artifact", value: f.itemName || "Not yet recorded" },
        { label: "Respectful interpretation", value: f.interpretation ? "Interpretation drafted" : "Not yet recorded" },
      ...
      sections: [
        ...
        {
          label: "Teachings and values",
          value: f.values || "No teachings and values section recorded yet."
        },
        {
          label: "Respectful interpretation",
          value: f.interpretation || "No respectful interpretation section recorded yet."
        },
        {
          label: "Bibliography",
          list: [
  ...
  function renderHeader() {
    ...
    refs.stepCode.textContent = `Chapter 5 · Step ${state.activeStep + 1}`;
  ...
  function renderIdentification() {
    return `
      <div class="step-section">
        ${renderField("Symbol or artwork", "itemName", "Example: Dharma Wheel, Lotus Flower, Stupa, Sand Mandala")}
      </div>
    `;
  }
    ...
    return `
      <div class="step-section">
        ${renderTextArea(
          ...
          "You can include physical details, setting, function, ritual use, meditation use, or visual symbolism."
        )}
      </div>
    `;
  }
    ...
    return `
      <div class="step-section">
        ${renderTextArea(
          ...
          "values",
          "Explain what Buddhist teachings or values it communicates.",
          "You can discuss impermanence, enlightenment, the Noble Eightfold Path, compassion, mindfulness, or interdependence."
        )}
      </div>
    `;
  }
    ...
    return `
      <div class="step-section">
        ${renderTextArea(
          ...
          "interpretation",
          "Describe one reason a respectful interpretation matters.",
          "Explain why context, sensitivity, and accurate understanding matter when someone outside Buddhism interprets the symbol or artwork."
        )}
      </div>
    `;
  }
...
<body>
  <div class="report-page">
    ...
    <header class="report-head">
      <div>
        <p class="report-code">Chapter 5 Report</p>
  ...
  function renderPanel() {
    switch (STEPS[state.activeStep].id) {
      ...
      case "values":
        ...
        return;
      case "interpretation":
        refs.panel.innerHTML = renderInterpretation();
        return;
      case "bibliography":
