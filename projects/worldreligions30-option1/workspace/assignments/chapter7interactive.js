(function () {
  const STORAGE_KEY = "worldreligions30-option1.assignment.chapter7interactive";
  const HEIGHT_MESSAGE = "wr30-assignment-height";
  const FRAME_KEY = "chapter7interactive";
  const STEPS = [
    {
      id: "identification",
      title: "Artifact Identification",
      marks: 2,
      copy: "Identify the Christian artwork, hymn, symbol, or monument you selected before you begin the explanation."
    },
    {
      id: "description",
    ...
    },
    {
      ...
      marks: 4,
      copy: "Explain what Christian beliefs or values it communicates."
    },
    {
      id: "culture",
      title: "Contemporary Culture",
      marks: 3,
      copy: "Describe one reason it may still matter in contemporary culture."
    },
    {
      id: "bibliography",
      title: "Bibliography",
  ...
  function getCompletedStepCount() {
    ...
    return [
      ...
      Boolean(f.itemName && f.description && f.values && f.culture)
  ...
  function getReportData() {
    const f = state.fields;
    const checklist = [
      { label: "Selected artwork, hymn, symbol, or monument recorded", complete: Boolean(f.itemName) },
      { label: "Detailed description recorded", complete: Boolean(f.description) },
      { label: "Christian beliefs or values explained", complete: Boolean(f.values) },
      { label: "Contemporary culture section explained", complete: Boolean(f.culture) },
    ...
    return {
      ready,
      title: f.itemName || "Unnamed Christian artwork or symbol",
      subtitle: f.itemName ? `Christian art, music, or symbol study: ${f.itemName}` : "Select a Christian artwork, hymn, symbol, or monument to build the report.",
      intro: "A Chapter 7 study report built from the notes below. Review the wording, then generate the printable version from this page.",
      meta: [
        { label: "Selected artifact", value: f.itemName || "Not yet recorded" },
        { label: "Contemporary culture", value: f.culture ? "Culture note drafted" : "Not yet recorded" },
      ...
      sections: [
        ...
        },
        {
          ...
          value: f.culture || "No contemporary culture section recorded yet."
        },
        {
          label: "Bibliography",
          list: [
  ...
  function renderHeader() {
    ...
    refs.stepCode.textContent = `Chapter 7 · Step ${state.activeStep + 1}`;
  ...
  function renderIdentification() {
    return `
      <div class="step-section">
        ${renderField("Artwork, hymn, symbol, or monument", "itemName", "Example: The Cross, Amazing Grace, St. Peter's Basilica, The Last Supper")}
      </div>
    `;
  }
    ...
    return `
      <div class="step-section">
        ${renderTextArea(
          ...
          "You can include visual details, origins, creator, setting, use in worship, or role in Christian life."
        )}
      </div>
    `;
  }
    ...
    return `
      <div class="step-section">
        ${renderTextArea(
          "Beliefs and values",
          "values",
          "Explain what Christian beliefs or values it communicates.",
          "You can discuss salvation, resurrection, sacrifice, grace, Trinity, devotion, hope, or service."
        )}
      </div>
    `;
  }
    ...
    return `
      <div class="step-section">
        ${renderTextArea(
          ...
          "culture",
          "Describe one reason it may still matter in contemporary culture.",
          "Explain how it continues to influence, inspire, symbolize, or remain visible in public or personal life today."
        )}
      </div>
    `;
  }
...
<body>
  <div class="report-page">
    ...
    <header class="report-head">
      <div>
        <p class="report-code">Chapter 7 Report</p>
  ...
  function renderPanel() {
    switch (STEPS[state.activeStep].id) {
      ...
      case "values":
        ...
        return;
      case "culture":
        refs.panel.innerHTML = renderCulture();
        return;
      case "bibliography":
