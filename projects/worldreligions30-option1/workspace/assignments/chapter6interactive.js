(function () {
  const STORAGE_KEY = "worldreligions30-option1.assignment.chapter6interactive";
  const HEIGHT_MESSAGE = "wr30-assignment-height";
  const FRAME_KEY = "chapter6interactive";
  const STEPS = [
    {
      id: "identification",
      title: "Artifact Identification",
      marks: 2,
      copy: "Identify the Jewish holy day, symbol, or ritual object you selected before you begin the explanation."
    },
    {
      id: "description",
    ...
    {
      id: "values",
      title: "Beliefs and Memories",
      marks: 4,
      copy: "Explain what beliefs, memories, or values it communicates."
    },
    {
      id: "connection",
      title: "Cross-Generational Connection",
      marks: 3,
      copy: "Describe one way it helps connect Jewish identity across generations."
    },
    {
      id: "bibliography",
      title: "Bibliography",
  ...
  function getCompletedStepCount() {
    ...
    return [
      ...
      Boolean(f.values),
      Boolean(f.connection),
      Boolean(f.source1 && f.source2),
      Boolean(f.itemName && f.description && f.values && f.connection)
  ...
  function getReportData() {
    const f = state.fields;
    const checklist = [
      { label: "Selected holy day or symbol recorded", complete: Boolean(f.itemName) },
      { label: "Detailed description recorded", complete: Boolean(f.description) },
      { label: "Beliefs, memories, or values explained", complete: Boolean(f.values) },
      { label: "Cross-generational connection explained", complete: Boolean(f.connection) },
    ...
    return {
      ready,
      title: f.itemName || "Unnamed Jewish holy day or symbol",
      subtitle: f.itemName ? `Jewish holy day or symbol study: ${f.itemName}` : "Select a Jewish holy day, symbol, or ritual object to build the report.",
      intro: "A Chapter 6 study report built from the notes below. Review the wording, then generate the printable version from this page.",
      meta: [
        { label: "Selected artifact", value: f.itemName || "Not yet recorded" },
        { label: "Identity connection", value: f.connection ? "Connection drafted" : "Not yet recorded" },
      ...
      sections: [
        ...
        {
          label: "Beliefs and memories",
          value: f.values || "No beliefs and memories section recorded yet."
        },
        {
          label: "Cross-generational connection",
          value: f.connection || "No cross-generational connection section recorded yet."
        },
        {
          label: "Bibliography",
          list: [
  ...
  function renderHeader() {
    ...
    refs.stepCode.textContent = `Chapter 6 · Step ${state.activeStep + 1}`;
  ...
  function renderIdentification() {
    return `
      <div class="step-section">
        ${renderField("Holy day, symbol, or object", "itemName", "Example: Passover, Menorah, Torah, Sabbath, Star of David")}
      </div>
    `;
  }
    ...
    return `
      <div class="step-section">
        ${renderTextArea(
          ...
          "You can include physical details, ritual use, historical setting, or how it is observed."
        )}
      </div>
    `;
  }
    ...
    return `
      <div class="step-section">
        ${renderTextArea(
          ...
          "values",
          "Explain what beliefs, memories, or values it communicates.",
          "You can discuss covenant, memory, Exodus, resilience, holiness, law, community, or identity."
        )}
      </div>
    `;
  }
    ...
    return `
      <div class="step-section">
        ${renderTextArea(
          ...
          "connection",
          "Describe one way it helps connect Jewish identity across generations.",
          "Explain how the holy day, symbol, or object preserves tradition, memory, belonging, or shared practice over time."
        )}
      </div>
    `;
  }
...
<body>
  <div class="report-page">
    ...
    <header class="report-head">
      <div>
        <p class="report-code">Chapter 6 Report</p>
  ...
  function renderPanel() {
    switch (STEPS[state.activeStep].id) {
      ...
      case "values":
        ...
        return;
      case "connection":
        refs.panel.innerHTML = renderConnection();
        return;
      case "bibliography":
