...
R=Path(__file__).resolve().parents[1];V=R/'verification';text=(R/'portable/Biology30_Chapter17.html').read_text();out={'environment':'Chromium about:blank, Map-backed storage double; not real-origin persistence or LMS testing.','tests':[],'errors':[]}
...
with sync_playwright()as p:
 ...
 page.set_content(source(),wait_until='domcontentloaded',timeout=45000);page.evaluate('document.fonts.ready');C=page.evaluate('BIOLOGY_HTML.getConfig()');key=page.evaluate('BIOLOGY_HTML.storageKey')or'biology30-chapter-17:html:v1'
 ...
 try:
  ...
  route('extension');model=page.locator('[data-extension-model]');rec('extension_model_initially_locked',model.get_attribute('data-locked')=='true');field=page.locator('[data-note-input="ch17-optional-extension"]');field.evaluate('(el)=>el.closest("details").open=true');field.fill('The comparison alone cannot isolate the proposed chemical effect. Hormone production and receptor response are distinct steps.');page.locator('[data-save-note="ch17-optional-extension"]').click();rec('extension_model_after_saved_attempt',model.get_attribute('data-locked')=='false')
  route('lesson-08');page.locator('#lesson-08 [data-open-pdf]').first.click();subtitle=page.locator('.textbook-subtitle').inner_text();rec('split_pdf_page_mapping','PDF page 18 of 38'in subtitle,subtitle);page.keyboard.press('Escape')
  # All 11 checks have resolvable response ids/options.
  rec('required_check_schema',len(C['checks'])==15 and all(len(q['mc'])==(6 if n==14 else 2)and len(q['writing'])==2 and all(x['answer']in x['options']for x in q['mc'])for n,q in enumerate(C['checks'])))
  rec('optional_work_progress_isolated',page.locator('[data-progress-count]').inner_text().startswith('0 of 15'))
