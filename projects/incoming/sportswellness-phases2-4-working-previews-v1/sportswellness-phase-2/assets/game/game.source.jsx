    ...
    <div className="min-h-screen bg-zinc-950 text-zinc-100 font-sans p-0 md:p-4 xl:p-6 selection:bg-lime-400 selection:text-black">
      ...
      >
        <div className="bg-black p-4 border-b border-zinc-800 flex flex-col md:flex-row justify-between items-center gap-4 relative z-50">
          ...
          {(gameState === 'playing' || gameState === 'win') && (
            <div className="flex gap-4 font-mono text-xs uppercase tracking-widest bg-zinc-900 px-4 py-2 border border-zinc-800 rounded">
              ...
              <div className="flex flex-col items-center border-r border-zinc-800 pr-4 pl-2">
                <span className="text-zinc-500 text-[9px] mb-1">Game interference</span>
        ...
        >
          <div className="absolute top-0 left-0 w-full h-[25%] bg-gradient-to-b from-lime-400/10 to-transparent border-b border-lime-400/50 group z-30">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none transition-transform group-hover:scale-105 duration-700">
              ...
              <p className={`text-[9px] text-lime-400/70 font-mono uppercase tracking-[0.3em] mt-1 transition-opacity ${activeEvent ? 'opacity-0' : 'opacity-100'}`}>
                Upper-area pointer time raises the game meter
              </p>
            </div>
            ...
            {currentEvent && LeftIcon && RightIcon && (
              <div className="absolute inset-0 flex items-center justify-between px-4 md:px-8 pointer-events-none">
                <button
                  onClick={(event) => { event.stopPropagation(); handleEventChoice(currentEvent.left.id); }}
                ...
                <button
                  onClick={(event) => { event.stopPropagation(); handleEventChoice(currentEvent.right.id); }}
          ...
          >
            {gameState === 'playing' && targets.map((target) => {
              ...
              return (
                <div
                  key={target.id}
                  onClick={(event) => handleHitTarget(target.id, event)}
            ...
            {shieldActive && (
              <div className="absolute inset-0 border-4 border-lime-400/50 pointer-events-none bg-lime-400/10 shadow-[inset_0_0_50px_rgba(0,255,202,0.08)]">
                <div className="absolute top-4 left-1/2 -translate-x-1/2 text-lime-400 font-mono text-xs uppercase tracking-widest flex items-center gap-2">
                  <ShieldAlert size={14} /> Game shield active
                </div>
              </div>
            )}
          </div>
          ...
          {gameState === 'intro' && (<div className="course-game-message"><h2>Process target game</h2><p>Hit the moving process targets to gain points. Missed targets and empty-area clicks cost points. Spending pointer time in the upper outcome area raises the game's interference meter.</p><p>Choice cards alter target sizes, timing and points through fictional rules. A quick point gain is not evidence that a harmful personal judgment is useful. The target is 500 game points.</p><p>Use Start above. The guided or untimed decision lab is the required conceptual practice.</p></div>)}

          {gameState === 'gameover' && (<div className="course-game-message"><h2>Round complete</h2><p>{causeOfDeath}</p><p>The game records inputs, timing and programmed consequences. It does not establish a cause of real performance or measure your psychology.</p><p>Review one rule in the debrief, or use Restart above.</p></div>)}
