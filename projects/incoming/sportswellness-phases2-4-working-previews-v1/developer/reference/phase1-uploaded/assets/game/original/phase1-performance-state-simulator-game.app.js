...
function PerformanceStateStimulatorGame() {
  ...
  const endGame = useCallback((reason) => {
    ...
    setArousal(Math.round(arousalRef.current));setTrackingPct(Math.round(lockFramesRef.current));setTimeElapsed(timeRef.current);setScore(Math.floor(scoreRef.current));
    audioCtxRef.current?.suspend();
    if (attemptRef.current && !reportedRef.current) {
      reportedRef.current=true;
      parent.postMessage({schemaVersion:1,activityId:'phase1-performance-state-simulator-game',type:'attempt',payload:{attemptId:attemptRef.current,status:['navigation','restart','round-ended'].includes(reason)?'interrupted':'completed',score:Math.floor(scoreRef.current),elapsedMs:Math.min(3600000,Math.round(timeRef.current*1000)),arousal:Math.max(0,Math.min(100,Math.round(arousalRef.current))),trackingPercent:Math.max(0,Math.min(100,Math.round(lockFramesRef.current))),pace:paceRef.current==='high'?'intense':paceRef.current,roundEndReason:reason,at:new Date().toISOString()}},messageOrigin==='null'?'*':messageOrigin);
    }
  }, []);

  useEffect(()=>{
    const hidden=()=>{if(document.hidden&&gameStateRef.current==='playing'){gameStateRef.current='paused';setGameState('paused');audioCtxRef.current?.suspend();}};
    const message=event=>{const d=event.data;if(event.source!==parent||event.origin!==messageOrigin||!d||d.schemaVersion!==1||d.activityId!=='phase1-performance-state-simulator-game'||d.type!=='control')return;
      if(d.action==='pause'&&gameStateRef.current==='playing'){gameStateRef.current='paused';setGameState('paused');audioCtxRef.current?.suspend();}
      if(d.action==='resume'&&gameStateRef.current==='paused'){gameStateRef.current='playing';setGameState('playing');}
      if(d.action==='stop'&&attemptRef.current&&!reportedRef.current)endGame('navigation');};
    document.addEventListener('visibilitychange',hidden);window.addEventListener('message',message);

    return()=>{document.removeEventListener('visibilitychange',hidden);window.removeEventListener('message',message);};
  },[endGame]);
  useEffect(()=>{if(gameState!=='playing')audioCtxRef.current?.suspend();},[gameState]);
  ...
  useEffect(() => {
    ...
    const update = (time) => {
      ...
      if(arousalRef.current<=MIN_AROUSAL){endGame('lower-limit');return;}
      if(arousalRef.current>=MAX_AROUSAL){endGame('upper-limit');return;}
  ...
  const initGame = () => {
    if(['playing','paused'].includes(gameState))endGame('restart');
    attemptRef.current=globalThis.crypto?.randomUUID?.()||`game-${Date.now()}`;reportedRef.current=false;
    setScore(0);
    setTimeElapsed(0);
    setArousal(50);
    setTrackingPct(100);
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-100 font-sans p-0 md:p-4 xl:p-6 selection:bg-lime-400 selection:text-black">
      <div className="w-full min-h-screen md:min-h-[calc(100vh-32px)] bg-zinc-900 md:rounded-xl shadow-2xl overflow-hidden border border-zinc-800 flex flex-col relative">
        <div className="bg-black p-4 border-b border-zinc-800 flex flex-col md:flex-row justify-between items-center gap-4 relative z-50">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-zinc-500 font-mono uppercase tracking-[0.2em] mt-1">Fictional activation and attention game</p>
            </div>
          </div>
          ...
          {gameState === 'playing' && (
            ...
            {gameState === 'intro' && (
              <div className="relative bg-zinc-950/90 backdrop-blur-sm z-50 flex flex-col items-center justify-center p-8 text-center border-t-4 border-lime-400">
                <h2 className="text-4xl font-black text-white uppercase tracking-tighter mb-4">The <span className="text-lime-400">Activation</span> Balancer</h2>
                <p className="text-zinc-400 max-w-xl text-sm leading-relaxed mb-8">
                  This optional chase game uses fictional activation and target-tracking variables. Move your pointer near the moving target. The 35–65 practice band is a game setting, not your personal optimum.
                  <br /><br />
                  The <strong>Pace of the Match</strong> shifts dynamically. During an <span className="text-red-400 font-bold">Intense Pace</span>, the target flies and pressure builds, raising the game variable. During a <span className="text-amber-400 font-bold">Lull</span>, the target crawls and the game variable falls.
                </p>
                ...
                <div className="flex gap-8 mb-10 text-left flex-wrap justify-center">
                  <div className="bg-zinc-900 border border-zinc-800 p-4 rounded-lg">
                    ...
                    <div className="text-zinc-500 text-[10px] uppercase mt-1">Lowers fictional activation by 15</div>
                  </div>
                  <div className="bg-zinc-900 border border-zinc-800 p-4 rounded-lg">
                    ...
                    <div className="text-zinc-500 text-[10px] uppercase mt-1">Raises fictional activation by 15</div>
                  </div>
                </div>
        ...
        <div className="game-controls" aria-label="Round controls">
          <button disabled={gameState!=='playing'} onClick={()=>setGameState('paused')}>Pause</button>
          <button disabled={gameState!=='paused'} onClick={()=>setGameState('playing')}>Resume</button>
          <button disabled={!['playing','paused'].includes(gameState)} onClick={()=>endGame('round-ended')}>End round</button>
          <button onClick={initGame}>Restart</button>
          <button aria-pressed={muted} onClick={()=>{mutedRef.current=!muted;setMuted(!muted);if(!muted)audioCtxRef.current?.suspend();}}>{muted?'Unmute':'Mute'}</button>
          {gameState==='paused' && <span>Paused. Resume when you are ready.</span>}
        </div>
            {gameState === 'gameover' && (
              <div className="relative bg-zinc-950/95 backdrop-blur-sm z-50 flex flex-col items-center justify-center p-8 text-center border-t-4 border-red-500">
                <div className="inline-block bg-red-500 text-white px-3 py-1 text-xs font-black uppercase tracking-[0.3em] mb-6">
                  Round summary
                </div>
                <h2 className="text-5xl font-black text-white uppercase tracking-tighter mb-4">Round ended</h2>
                ...
                <div className="bg-zinc-900 border border-zinc-800 p-8 max-w-lg w-full mb-8 text-center shadow-2xl flex gap-8 justify-center">
                  <div>
                    <h3 className="text-zinc-500 font-mono uppercase text-[10px] tracking-widest mb-2">Round time</h3>
                ...
                <div className="bg-zinc-900 border border-zinc-800 p-6 max-w-2xl w-full text-left mb-8">
                  <h3 className="text-white font-bold uppercase text-xs tracking-wider mb-4 border-b border-zinc-800 pb-2">What happened</h3>
                  <p className="text-zinc-400 text-sm leading-relaxed">
                    {causeOfDeath==='tracking-loss' ? `The cursor did not stay near the target. Final fictional activation: ${arousal}, ${arousal>=35&&arousal<=65?'inside':'outside'} this game's practice band. Tracking loss does not establish anything about your real anxiety or regulation skill.` : causeOfDeath==='upper-limit' ? 'The fictional activation variable reached its upper limit. This game does not model cognitive anxiety and does not demonstrate the Catastrophe Model.' : causeOfDeath==='lower-limit' ? 'The fictional activation variable reached its lower limit. This does not establish apathy or a personal psychological state.' : 'You ended or left this round. Your partial attempt is saved with the course work.'}
                  </p>
                </div>
        ...
        <div style={{display:['intro','gameover'].includes(gameState)?'none':undefined}} className="flex flex-col md:flex-row md:flex-1 md:min-h-[680px]">
          <div className="w-full md:w-64 bg-zinc-950 border-b md:border-b-0 md:border-r border-zinc-800 p-4 md:p-6 flex flex-col relative overflow-hidden shrink-0">
            ...
            <div className="relative z-10 flex-1 flex flex-col">
              ...
              <div className="relative w-full aspect-[4/3] max-h-[260px] md:aspect-square md:max-h-none bg-zinc-900/50 border-b border-l border-zinc-700 mb-5 md:mb-6 rounded-tr">
                ...
                <div className="absolute -bottom-5 left-0 text-[8px] text-zinc-600 font-mono">Low</div>
                <div className="absolute -bottom-5 right-0 text-[8px] text-zinc-600 font-mono">High</div>
              ...
              <div className="mt-4 space-y-3">
                <button
                  ...
                  disabled={gameState !== 'playing' || cooldowns.breathe > 0}
                ...
                <button
                  ...
                  disabled={gameState !== 'playing' || cooldowns.activate > 0}
