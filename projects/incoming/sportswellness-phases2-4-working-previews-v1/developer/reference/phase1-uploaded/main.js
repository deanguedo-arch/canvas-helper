...
const fresh=()=>({version:2,courseId:meta.courseId,drafts:{},noteEntries:[],reviewed:{},topic:'start',question:0,buildStep:0,ratings:{},practice:{},firstChecks:{},revealed:{},appraisal:null,lab:{round:0,attempts:[],locked:false,last:null},checkpointArchive:[],checkpoint:{contentVersion:'phase1-checkpoint-2026-09-17-r2',selections:{},attempts:[],bestScore:null,lastSubmittedFingerprint:null},game:{attempts:[],interrupted:null},untimedPractice:{strategy:'',reflection:'',cases:[{choice:'',reason:''},{choice:'',reason:''}],transfer:''},gameDebrief:'',updatedAt:null});
...
function normalize(raw) {
 ...
 if(Array.isArray(raw.checkpointArchive))s.checkpointArchive=raw.checkpointArchive.filter(a=>obj(a)&&typeof a.contentVersion==='string'&&Array.isArray(a.attempts));
 if(obj(raw.checkpoint)&&raw.checkpoint.contentVersion!==s.checkpoint.contentVersion&&Array.isArray(raw.checkpoint.attempts)&&raw.checkpoint.attempts.length)s.checkpointArchive.push({contentVersion:str(raw.checkpoint.contentVersion,80),attempts:raw.checkpoint.attempts});
 ...
 if(obj(raw.untimedPractice)){s.untimedPractice.strategy=str(raw.untimedPractice.strategy,80);s.untimedPractice.reflection=str(raw.untimedPractice.reflection);s.untimedPractice.transfer=str(raw.untimedPractice.transfer);if(Array.isArray(raw.untimedPractice.cases))raw.untimedPractice.cases.slice(0,2).forEach((a,i)=>{if(obj(a))s.untimedPractice.cases[i]={choice:['up','down','hold'].includes(a.choice)?a.choice:'',reason:str(a.reason)};});}
...
function saveUI(changed=null){
 ...
 const footer=$('#school-local-status');if(footer&&!storageOK)footer.textContent='Browser storage is unavailable. Work remains in this tab. Download a backup in the course guide before leaving.';
...
function saveNow(){
 ...
 pending=false;saveUI();return storageOK;
}
...
function syncNav(){
 ...
 menuButton.setAttribute('aria-label',`${menuAction} · ${reviewedTotal} of ${sections.length} topics reviewed`);
...
function navigate(id,{historyMode='push',focus=true}={}){
 ...
 saveNow();
 document.dispatchEvent(new CustomEvent('phase1-route-activated',{detail:{route:section.id}}));
...
const reusePairs={'p1_cue_inst':'attention-cue','p1_goal_proc':'goal-rewrite'};
for(const [target,source]of Object.entries(reusePairs)){
 const to=fieldMap.get(target),from=fieldMap.get(source);if(!to||!from)continue;
 const button=document.createElement('button');button.type='button';button.className='quiet-button';button.textContent='Reuse my earlier response';
 button.addEventListener('click',()=>{if(!from.value.trim()){navigate(from.id);return;}if(to.value.trim()&&!confirm('Replace this field with your earlier response?'))return;to.value=from.value;changed(to);saveNow();});to.after(button);
}
...
$('#save-note-entry')?.addEventListener('click',()=>{
 ...
 if(state.noteEntries.length>=50){status('You have 50 saved notes. Keep a backup, then delete an entry before adding another. Your draft remains here.');return;}
...
function exportDoc(mode){
 ...
 if(!only){
  ...
  content+='<h2>Optional game rounds</h2>';state.game.attempts.forEach(a=>content+=textBlock(niceDate(a.at),`${a.roundEndReason} · ${a.elapsedMs/1000} seconds · fictional activation ${a.arousal} · tracking ${a.trackingPercent}%`));
  content+=textBlock('Game debrief',state.gameDebrief);
  content+='<h2>Untimed regulation practice</h2>'+textBlock('Selected first move',state.untimedPractice.strategy)+textBlock('Reasoning',state.untimedPractice.reflection);state.untimedPractice.cases.forEach((a,i)=>content+=textBlock(`Additional case ${i+1} · ${a.choice}`,a.reason));content+=textBlock('Untimed transfer explanation',state.untimedPractice.transfer);if(state.checkpointArchive.length){content+='<h2>Earlier checkpoint versions</h2>';state.checkpointArchive.forEach(a=>content+=textBlock(a.contentVersion,JSON.stringify(a.attempts)));}
 }
 ...
 if(only)download('My_Regulation_Engine_Phase1.html',html,'text/html;charset=utf-8');
  ...
  if(!printWindow){download('My_Work_Phase1.html',html,'text/html;charset=utf-8');status('The print window was blocked. Open the downloaded My Work file, then select Print / Save as PDF.');return;}
...
function validateBackup(raw){
 ...
 for(const key of ['drafts','reviewed','firstChecks','practice','lab','checkpoint','game','untimedPractice'])if(!obj(raw[key]))throw new Error('Invalid '+key+' records.');
 if(Object.entries(raw.drafts).some(([k,v])=>!fieldMap.has(k)||typeof v!=='string'||v.length>5000))throw new Error('Invalid response text.');
 if(!Array.isArray(raw.noteEntries)||raw.noteEntries.length>50||raw.noteEntries.some(a=>!obj(a)||typeof a.id!=='string'||typeof a.text!=='string'||typeof a.at!=='string'))throw new Error('Invalid note entries.');
 for(const [key,max] of Object.entries({'phase1-q1':3,'phase1-q2':3,'phase1-q3':3,'phase1-q4':3,'phase1-q5':3,'phase1-q6':3,'phase1-q7':3,'phase1-q8':3,'phase1-q9':3,'phase1-q10':3}))if(raw.checkpoint.selections?.[key]!==undefined&&(!Number.isInteger(raw.checkpoint.selections[key])||raw.checkpoint.selections[key]<0||raw.checkpoint.selections[key]>max))throw new Error('Invalid checkpoint choices.');
 for(const [key,value]of Object.entries(raw.firstChecks))if(!obj(value)||typeof value.text!=='string'||typeof value.at!=='string')throw new Error('Invalid review snapshots.');
 for(const value of Object.values(raw.practice))if(!Array.isArray(value)||value.some(a=>!obj(a)||!Number.isInteger(a.choice)||typeof a.correct!=='boolean'||typeof a.at!=='string'))throw new Error('Invalid formative attempts.');
 for(const group of ['lab','checkpoint','game'])if(!Array.isArray(raw[group].attempts)||raw[group].attempts.some(a=>!obj(a)||typeof a.at!=='string'))throw new Error('Invalid '+group+' attempts.');
 if(raw.lab.attempts.some(a=>!['up','down','hold'].includes(a.action)||!Number.isInteger(a.round)||a.round<0||a.round>2||typeof a.prediction!=='string'))throw new Error('Invalid guided cases.');
 if(!obj(raw.checkpoint.selections)||raw.checkpoint.attempts.some(a=>typeof a.attemptId!=='string'||!Number.isInteger(a.score)||a.score<0||a.score>10||a.percent!==a.score*10||!obj(a.answers)))throw new Error('Invalid checkpoint attempt.');
 if(raw.game.attempts.some(a=>typeof a.attemptId!=='string'||!['completed','interrupted'].includes(a.status)||['score','elapsedMs','arousal','trackingPercent'].some(k=>!Number.isInteger(a[k])||a[k]<0)||a.arousal>100||a.trackingPercent>100))throw new Error('Invalid game summary.');
 if(typeof raw.untimedPractice.strategy!=='string'||typeof raw.untimedPractice.reflection!=='string')throw new Error('Invalid untimed work.');
 if(raw.untimedPractice.cases!==undefined&&(!Array.isArray(raw.untimedPractice.cases)||raw.untimedPractice.cases.some(a=>!obj(a)||typeof a.reason!=='string'||!['','up','down','hold'].includes(a.choice))))throw new Error('Invalid untimed cases.');
}
...
$('#school-save-exit')?.addEventListener('click',()=>{
 ...
 status(storageOK?'Saved in this browser.':'Browser storage is unavailable. Keep this tab open and download a backup before leaving.');
});
