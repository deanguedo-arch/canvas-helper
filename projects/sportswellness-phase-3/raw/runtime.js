...
(() => {
  ...
  function acceptGameMessage(event){
    if(!gameFrame||event.source!==gameFrame.contentWindow||event.origin!==location.origin)return false;
    const d=event.data;
    if(!plain(d)||d.channel!=='sportswellness-game'||d.activityId!==config.gameId||d.version!==1||d.session!==gameSession)return false;
    if(d.type==='ready'){
      gameFrame.contentWindow.postMessage({channel:'sportswellness-control',activityId:config.gameId,version:1,session:gameSession,command:'init'},location.origin==='null'?'*':location.origin);if(state.topic!=='performance-game'||document.hidden)interruptGame('Activity is not visible');return true;
    }
