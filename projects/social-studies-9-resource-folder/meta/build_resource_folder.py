...
SOURCE_ZIP = Path(
    os.environ.get("SOCIAL_STUDIES_9_SOURCE_ZIP")
    or r"c:\Users\dean.guedo\Downloads\D2LExport_151052_25-26 _ Social Studies 9 _ Per 1(A) _ Sec 1_202652100.zip"
)
...
def read_manifest(zip_file: zipfile.ZipFile) -> tuple[list[Unit], dict[str, str]]:
    ...
    for unit_index, unit_node in enumerate(organization.findall("ims:item", IMS_NS), start=1):
        ...
        if hidden:
            folder += "_HIDDEN"
        resources: list[ResourceItem] = []
        for resource_index, item_node in enumerate(unit_node.findall("ims:item", IMS_NS), start=1):
            identifier_ref = item_node.attrib.get("identifierref", "")
            href = resources_by_id.get(identifier_ref, "")
            if not href:
                continue
            item_title = text_of(item_node, "ims:title") or Path(href).stem
            source_name = Path(href).name
            output_name = f"{resource_index:02d}_{source_name}"
...
def write_readme(units: list[Unit], unreferenced: list[str]) -> None:
    lines = [
        "# Social Studies 9 Resource Folder",
        "",
        "Resources are grouped by the unit order in the Brightspace manifest.",
        "Unit 4 was hidden in the source course, so its folder name includes `_HIDDEN`.",
        "",
        "## Unit Folders",
        "",
    ]
    ...
    if unreferenced:
        lines.append("These Office files were present in the ZIP but were not referenced by the Brightspace manifest:")
        lines.append("")
...
def zip_resource_folder() -> Path:
    zip_path = EXPORTS / "social-studies-9-resource-folder.zip"
    if zip_path.exists():
        zip_path.unlink()
...
def build() -> dict:
    ...
    with zipfile.ZipFile(SOURCE_ZIP) as zip_file:
        ...
        source_office_files = [
            ...
            if Path(name).suffix.lower() in OFFICE_EXTENSIONS and not name.endswith("/")
        ]
        unreferenced = sorted(name for name in source_office_files if name not in referenced_hrefs)
        if unreferenced:
            unreferenced_dir = RESOURCE_FOLDER / "unreferenced-source-files"
            for name in unreferenced:
                copy_from_zip(zip_file, name, unreferenced_dir / Path(name).name)
    ...
    audit = {
        ...
        "projectSlug": "social-studies-9-resource-folder",
        ...
        "sourceOfficeFileCount": len(unreferenced) + sum(len(unit.resources) for unit in units),
