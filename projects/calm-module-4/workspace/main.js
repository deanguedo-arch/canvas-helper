...
function getActiveTabProgress() {
  if (!state.tabs.length) return 0;
  return Math.round(((getActiveTabIndex() + 1) / state.tabs.length) * 100);
}

function moveActiveTab(offset) {
  if (!state.tabs.length) return;
  const currentIndex = getActiveTabIndex();
  const nextIndex = Math.min(state.tabs.length - 1, Math.max(0, currentIndex + offset));
  ...
  return `
    <main class="shell">
      <section class="loading card">
        ...
        <p>Fetching the draft payload and building the section map.</p>
      </section>
    </main>
  `;
}
  ...
  return `
    <main class="shell">
      <section class="error card">
        ...
        <a class="button button--secondary" href="./index.html">Retry workspace</a>
      </section>
    </main>
  `;
}
...
function heroHtml() {
  ...
  return `
    <header class="hero">
      <div class="hero-top">
        <div>
          <div class="eyebrow">CALM workspace</div>
          <h1>${escapeHtml(payload.title || "CALM Module 4")}</h1>
          <p class="lede">
            A workbook-style workspace for Career Exploration & Portfolio, shaped with the same warm clay-card language used in the CALM Module 2 reference.
            The left rail follows the draft's own tab group, and the main panel renders the real block data from Module 4.
          </p>
          <div class="chips">
          ...
          <div class="hero-progress">
            <div class="hero-progress__row">
              <span class="hero-progress__label">Section position</span>
              <span class="hero-progress__meta">${state.tabs.length ? `${activeIndex + 1} of ${state.tabs.length}` : "0 of 0"}</span>
            </div>
            <div class="hero-progress__track">
              <div class="hero-progress__fill" style="width: ${activeProgress}%"></div>
            </div>
          </div>
        </div>
        ...
        <div class="meta">
          <div class="meta-grid">
            ...
            <div class="meta-card meta-card--actions">
              <p class="meta-card__label">Workspace controls</p>
              <div class="nav-row">
                <button type="button" class="nav-button nav-button--secondary" data-nav="prev">Previous</button>
                <button type="button" class="nav-button nav-button--primary" data-nav="next">Next</button>
              </div>
            </div>
          </div>
        </div>
  ...
  return `
    <aside class="sidebar">
      <section class="card">
        ...
        <p class="card-subtle">Use the section map to move through the workbook the same way we would in a CALM-style clay-card build.</p>
        <nav class="tabs" aria-label="CALM Module 4 sections">${tabs}</nav>
      </section>

      <section class="card">
        <h2 class="card-title">Block mix</h2>
        <p class="card-subtle">A quick glance at the types that make up the module.</p>
...
function renderApp() {
  ...
  root.querySelectorAll("[data-nav]").forEach((button) => {
    button.addEventListener("click", () => {
      moveActiveTab(button.getAttribute("data-nav") === "next" ? 1 : -1);
    });
  });
}
