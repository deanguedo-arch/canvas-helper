...
PROJECT_ROOT = REPO_ROOT / "projects" / "physics-30-quiz-image-export"
ZIP_PATH = Path(
    r"C:\Users\dean.guedo\Downloads\D2LCCExport_151128_25-26 _ S2 _ Physics 30 _ Per 1(A) _ Sec 3_20265711.zip"
)
...
OUTPUT_ZIP = EXPORT_DIR / "physics-30-brightspace-quiz-image-package.zip"
...
def build_package():
    ...
    with zipfile.ZipFile(ZIP_PATH) as zip_file:
        ...
        for quiz_number, qti_href in enumerate(qti_hrefs, start=1):
            ...
            folder_pngs = sorted(
                ...
                if name.startswith(f"{quiz_folder}/") and name.lower().endswith(".png")
            )
            referenced_unique = {ref.source_href for ref in all_refs if ref.source_href}
            unreferenced = [name for name in folder_pngs if name not in referenced_unique]
            for source_href in unreferenced:
                audit["unreferencedQuizImages"].append({"quizTitle": title, "sourceHref": source_href})
            ...
            quiz_record = {
                ...
                "sourceFolderImageCount": len(folder_pngs),
            ...
            quiz_index_rows.append(
                {
                    ...
                    "sourceFolderImages": len(folder_pngs),
    ...
    index_lines = [
        "# Physics 30 Brightspace Quiz Image Package",
        "",
    ...
    with zipfile.ZipFile(OUTPUT_ZIP, "w", compression=zipfile.ZIP_DEFLATED) as output_zip:
        for file_path in sorted(PACKAGE_DIR.rglob("*")):
            if file_path.is_file():
                arcname = f"physics-30-brightspace-quiz-image-package/{file_path.relative_to(PACKAGE_DIR).as_posix()}"
...
def verify_export(audit: dict[str, Any]) -> dict[str, Any]:
    ...
    if OUTPUT_ZIP.exists():
        with zipfile.ZipFile(OUTPUT_ZIP) as z:
            ...
            zip_ok = "physics-30-brightspace-quiz-image-package/README.md" in names
