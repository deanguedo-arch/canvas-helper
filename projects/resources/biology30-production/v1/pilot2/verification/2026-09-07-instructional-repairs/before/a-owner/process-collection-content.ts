...
import { CHAPTER_11_STUDY_TASKS, studyResponseId } from "./chapter-11-study.js";
import { ONLINE_STUDIES, ONLINE_MICROSCOPY, onlineStudyResponseId } from "./online-studies.js";
...
export type ProcessCollectionRecordV1 = {
  ...
  stateRef:
    ...
    | { kind: "model"; modelId: string; studyTasks?: Array<{ id: string; responseId: string; title: string; cells?: Array<{label:string;options:Record<string,string>}> }> }
...
export function buildProcessCollectionRegistry(input: ProcessCollectionRegistryInput): ProcessCollectionRegistryV1 {
  ...
  input.practice.forEach(({ routeId, item }, index) => {
    ...
    records.push({
      ...
      courseEvidence: { choices: { ...item.legacyChoices, ...item.choices }, answer: item.answer, rationale: item.rationale, feedback: item.feedback ?? {}, textbook: item.textbook }
    });
  });
  ...
  input.models.forEach((model, index) => {
    ...
    records.push({
      ...
      selectKey: model.id,
      stateRef: { kind: "model", modelId: model.id, studyTasks: [
        ...CHAPTER_11_STUDY_TASKS.filter(task => task.modelId === model.id).map(task => ({ id: task.id, responseId: studyResponseId(task.id), title: task.title })),
        ...ONLINE_STUDIES.filter(task=>task.modelId===model.id).map(task=>({
          id:task.id, responseId:onlineStudyResponseId(task.id), title:task.title,
          cells:task.rows.flatMap(row=>task.columns.map((column,index)=>({
            label:`${row.clue} / ${column}`,
            options:Object.fromEntries(task.options[index].map((label,choice)=>[String.fromCharCode(97+choice),label]))
          })))
        })),
        ...(model.id===ONLINE_MICROSCOPY.modelId?[{id:ONLINE_MICROSCOPY.id,responseId:ONLINE_MICROSCOPY.responseId,title:ONLINE_MICROSCOPY.title}]:[])
      ] },
