...
PROJECT_SLUG = "learning-strategies-15"
DATA_GLOBAL = "LEARNING_STRATEGIES_15_DATA"
SOURCE_REF_ROOT = "learning-strategies-15"
CONFIG = COURSES["learning-strategies15"]
...
SOURCE_ZIP = Path(os.environ.get(CONFIG.source_zip_env) or r"c:\Users\dean.guedo\Documents\ONLINE COURSES\Learning Strategies\Learning Strategies 15 (3 credit)\Learning Strategies 15 package\00_SOURCE_ZIP\D2LCCExport_149766_25-26 _ S1 _ Learning Strategies 15 (2018) _ Per 1_202651252.zip")
ASSET_ZIP = Path(
    os.environ.get("LEARNING_STRATEGIES_15_ASSET_ZIP")
    or r"c:\Users\dean.guedo\Downloads\D2LExport_68818_22-23 _ S2 _ Learning Strategies 15 (2018) _ Per 1_202652115.zip"
)
...
class LearningStrategiesShellBuilder:
    ...
    def prepare(self) -> None:
        ...
        styles = styles.replace("Forensics 25", "Learning Strategies 15")
...
"""
    ...
    def resolve_asset_href(self, src: str) -> str | None:
        ...
        if "/images/" in normalized:
            candidates.append("LS15_2019/" + normalized.split("/images/", 1)[1].join(["images/", ""]))
        if "/commonImages/" in normalized:
            candidates.append("LS15_2019/" + normalized.split("/commonImages/", 1)[1].join(["commonImages/", ""]))
        if normalized.startswith("../../images/"):
            candidates.append("LS15_2019/" + posixpath.normpath(normalized[6:]))
...
<head>
  ...
  <title>{escape(chapter["title"])} | Learning Strategies 15</title>
...
"""
  ...
  const readyType = "learning-strategies-15-module-progress-ready";
...
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>Learning Strategies 15</title>
  <link rel="preconnect" href="https://fonts.googleapis.com">
  <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
        ...
        <section class="progress-shell is-hero" aria-label="Course progress">
          <div class="progress-panel">
            <div class="progress-copy">
              ...
              <h2 id="course-title">Learning Strategies 15</h2>
...
"""
    ...
    def write_main_js(self) -> None:
        main_js = r'''(function () {
  const data = window.LEARNING_STRATEGIES_15_DATA || { course: {}, chapters: [], quizzes: [], assignments: [], library: [] };
  const PROJECT_SLUG = document.body?.dataset.projectSlug || "learning-strategies-15";
  const STORAGE_KEY = "learning-strategies-15.progress";
  const UI_KEY = "learning-strategies-15.ui";
  ...
  function syncChapterProgressFrame(chapterId, focusComponentId = "") {
    ...
    frame.contentWindow.postMessage({
      type: "learning-strategies-15-module-progress-sync",
      chapterId,
  ...
  window.addEventListener("message", (event) => {
    const payload = event.data;
    if (!payload || typeof payload !== "object") return;
    if (payload.type === "learning-strategies-15-module-progress-ready" && typeof payload.chapterId === "string") {
      syncChapterProgressFrame(payload.chapterId);
      return;
    }
    if (
      payload.type === "learning-strategies-15-module-progress-update"
  ...
  render();
    ...
    def write_project_json(self) -> None:
        ...
        project = {
            ...
            "generatedOutputs": [],
            "regenerateCommand": "python projects/learning-strategies-15/meta/build_forensics_style_course.py",
            "injectedComponents": [],
            ...
            "googleHosted": {
                "trackedStorageKeys": [
                    "learning-strategies-15.progress",
                    "learning-strategies-15.ui",
                ],
                "authMode": "google",
            },
...
<head>
<meta charset="utf-8">
<title>Learning Strategies 15 Brightspace Import Baseline</title>
</head>
<body>
<h1>Learning Strategies 15 Brightspace Import Baseline</h1>
