...
text=(R/'portable/Biology30_Chapter15.html').read_text()
...
with sync_playwright() as p:
 ...
 try:
  conf=page.evaluate('BIOLOGY_HTML.getConfig()');record('runtime_initialises',conf['chapter']==15, str(len(conf['checks']))+' required checks')
  ...
  sec=page.locator('[data-check-id="ch15-check-01"]');sec.evaluate('(el)=>el.closest("details").open=true')
  sec.locator('[data-start-check]').click();spec=conf['checks'][0]
  sec.locator('[data-check-answer]').first.click();record('blank_check_blocked',len(page.evaluate('BIOLOGY_HTML.getSnapshot().checks["ch15-check-01"].mc')[spec['mc'][0]['id']]['attempts'])==0)
  ...
  tn.locator('[data-transfer-submit]').click();record('transfer_blank_blocked',not page.evaluate('BIOLOGY_HTML.getSnapshot().transfer?.["ch15-transfer-01"]?.submissions?.length'))
  for q in t['mc']:page.locator(f'[data-transfer-choice="{q["id"]}"]').evaluate_all('(els,a)=>els.find(x=>x.value===a).click()',q['answer'])
  tn.locator('textarea').fill(t['writing']['model']);tn.locator('[data-transfer-submit]').click();first=page.evaluate('BIOLOGY_HTML.getSnapshot().transfer["ch15-transfer-01"].submissions[0]')
  q=t['mc'][0];wrong=next(x for x in q['options']if x!=q['answer']);page.locator(f'[data-transfer-choice="{q["id"]}"]').evaluate_all('(els,a)=>els.find(x=>x.value===a).click()',wrong);tn.locator('[data-transfer-submit]').click();later=page.evaluate('BIOLOGY_HTML.getSnapshot().transfer["ch15-transfer-01"]');record('transfer_first_submission_immutable',first==later['submissions'][0]and len(later['submissions'])==2)
