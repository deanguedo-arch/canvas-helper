...
### Optional Muse delegation

Canvas Helper can delegate bounded implementation work from Codex to the user's local Muse coding agent through `.agents/skills/muse-delegate/`. This is an operator workflow, not a learner runtime or Studio model-provider integration. The launcher records the source commit, rejects overlapping dirty source changes, creates a detached worktree outside the repository, optionally limits a large checkout to explicit sparse paths, runs Muse with its sandbox enabled, and verifies the resulting changed paths against an explicit allowlist. Ignored run manifests, transcripts, local circuit state, and the observed five-hour usage ledger live under `.runtime/muse-delegate/`. The ledger separates delegated prompts from their provider calls and token totals. Manual disable persists until re-enabled; a confirmed usage-limit failure closes the current observed window and opens a five-hour automatic cooldown because the Muse CLI exposes no quota-status endpoint. The retained worktree remains the reviewable source until Codex independently inspects and integrates the patch. Muse never owns canonical project metadata, Git publication, packaging, deployment, or release claims.
...
- `projects/resources/<science-family>/comparison-contract.json`: the versioned two-source Science comparison boundary created by `intake:science-comparison`. Named Brightspace resources are stored once under `_sources/`; all five faithful, optimized, and synthesis projects reference that shared library. The Biology 30 Unit A implementation is intentionally course-specific, transactionally rebuilds only blocked `proposal-only-v1` outputs, recreates the checksum-verified PDF notes as semantic HTML, and records source-slide and manual diagram/table provenance before any winner can be promoted.
- `projects/resources/biology30-unit-a-pilot/v2/production-contract.json`: the strict production-candidate contract created once by `intake:biology30-unit-a-v2`. It owns the exact 25-outcome map, 17-lesson/1,505-minute sequence, explicit Brightspace item and PDF-page selections, seven-artifact and 100-item blueprints, source/rights register, correction ledger, and review gates. `scripts/lib/biology30-unit-a/v2/` owns validation, transactional intake, authored rendering, local interaction/persistence runtime, staged candidate builds, and exact-build promotion. `build:biology30-unit-a-v2` verifies the approved contract and immutable sources, preserves the previous valid candidate on failure, and emits review reports without invoking the five-version builder. The Biology-specific figure audit renders every figure placement inside the exact candidate at desktop, tablet, and mobile widths, records geometry findings, and emits contact sheets; a separate exact-hash verifier requires an explicit all-sheets visual-inspection record before Gate 3 may claim full figure review. `projects/biology30-unit-a/` remains `blocked` / `proposal-only-v1` until `promote:biology30-unit-a-v2` validates explicit human scores and the exact candidate hash, adds invisible durable edit identities, removes proposal rebuild ownership, and transactionally establishes the frozen workspace as `direct-workspace-v1`. The same transaction marks all five comparison projects `reference-only`; it rolls back the production and pilot metadata together on failure.
- `projects/biology30-unit-a-pilot/`: the blocked, preview-only, direct-authored Unit A improvement sandbox. Its canonical learner source is `workspace/index.html`; `meta/textbook-integration.json` owns exact textbook/review mappings and corrections, while `prepare:biology30-unit-a-pilot:textbook` owns only normalized textbook/review PDFs and `meta/textbook-resource-report.json`. `meta/media-integration.json` accounts for every item in the three checksum-verified Unit A PowerPoints; `prepare:biology30-unit-a-pilot:media` owns only content-addressed deck copies, extracted authoring references, and media reports. `meta/source-visual-integration.json` owns the exact nine-image source trial; `prepare:biology30-unit-a-pilot:visuals` owns only the local `workspace/assets/source-visuals/**` derivatives and source-visual resource report. All preparers stage writes, refuse source drift, and never rewrite canonical HTML. Six detailed source plates replace exact redundant static figures, while three distinct source images remain supplemental; all non-duplicated semantic figures and interactions remain. Learner media provides accessible enlargement, nearby explanations, and privacy-enhanced optional-video previews that load only when their lesson or selected library entry is visible, never autoplay, leave hidden entries unloaded, and retain local summaries. `meta/improvement-ledger.json` records implemented rules and exact-build visual evidence without granting user acceptance or automatic Unit B-D rollout.
- `projects/resources/biology30-production/v1/`: the Biology-specific production boundary for Units B, C, and D. `intake:biology30-course` transactionally creates all three blocked proposals from the same checksum-verified Brightspace library, exact official outcome records, and 395-page notes disposition; duplicate targets or any failed write roll back the whole family. The explicit unit contracts under `units/unit-{b,c,d}/` own 49 lessons, 92 outcomes, 30 artifacts, and 318 practice items. `scripts/lib/biology30-course/v1/` owns their authored content, local models, compact persistence, strict staged rebuilds, source/rights audits, and exact-build acceptance validation. `build:biology30-course` replaces only the named blocked candidate and preserves the previous valid workspace on failure. Each lesson explicitly maps both its semantic scientific figure and its keyboard-operable case-board interaction to one of nine scientific presentation forms; build validation rejects missing mappings, a legacy generic dropdown, or fewer than seven forms in any unit. `audit:biology30-course-production:visual` refuses workspace/build drift, renders every learner route plus all 49 figures and 49 interactions at desktop, tablet, and mobile widths, fails measurable layout defects, and records exact route, figure, and interaction contact-sheet hashes. After an explicit teacher decision, `record:biology30-course-acceptance` rejects stale, incomplete, blocker-bearing, or sub-95 records and transactionally records the scores without changing the blocked proposal ownership or authorizing Studio Edit, promotion, export, upload, or publication. A separately authorized promotion implementation is still required.
- Biology B/C/D Pilot 2 preparation extends that owner with `scripts/prepare-biology30-course-resources.py` and `scripts/lib/biology30-course/v1/pilot2-contract.ts`. Content-addressed baseline/intake/relationship packets live in `v1/pilot2/`; authored draft topic contracts live in `v1/units/unit-{b,c,d}/pilot2-contract.json`. The existing 49-lesson candidate remains unchanged until new contracts and the opt-in renderer are complete. Draft structure, source availability, academic clearance, teacher acceptance and Studio activation remain separate states.
- The Next Step shell supports an opt-in `authored` presentation boundary and `self-contained` chrome assets. Biology supplies the complete semantic lesson body plus local fonts/icons while the shell retains routing, navigation, progress, completion, and persistence hooks. Existing `document` and `ela30` presentations retain their prior defaults.
...
- SCORM 2004 / 1.2: copies the workspace, injects `scripts/lib/scorm.ts`, and emits LMS packages plus manifests.
  `scripts/lib/scorm-actions.ts` owns opt-in SCORM 2004 ungraded interaction aggregates, stable row reuse, action collection and active/foreground page-time summaries. Native LMS interactions retain counters across launches without adding action histories to suspend data. It does not copy writing into analytics or change grades/completion.
  `scripts/lib/scorm-tracking.ts` owns the versioned hash-page tracking contract, conservative existing-shell detection, bookmark/progress mapping, and active-time accounting. The exporter validates optional canonical `workspace/scorm-tracking.json` and emits `scorm-tracking-report.json` in the generated package. The bridge restores state before course scripts, commits progress and session time alongside responses, and reports save failures visibly. Required work comes from the declared completion list, never page visits. Science builders with packed or computed state must expose their actual completion IDs through the contract; no Science regeneration or release is implied. Workspace contracts already participate in export freshness through the workspace fingerprint.
