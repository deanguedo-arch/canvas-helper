...
Studio API routes and course preview routes have separate browser origins. The preview server is a separately allocated loopback port that serves only `GET`/`HEAD` preview assets and the early-injected preview bridge. It has no Studio APIs, denies `display-capture` through Permissions Policy, pins its allowed Studio parent through `frame-ancestors`, validates contained real paths, and returns 404 for every other route. Studio never reads a preview DOM. Embedded scroll, inspect, and selection events travel through a bounded private `MessageChannel` that is re-established after each frame load. A full-page preview opened by Studio uses a separate one-time token and private channel; the early-injected bridge sends the channel only to the exact Studio origin and clears the popup's `window.opener` before any course script executes.
...
The visible Inspector is intentionally a teacher-facing annotation surface rather than a source dashboard. It shows the selected course text, one change note, optional screenshot controls, and a temporary Review Set of at most five items. Source files, ownership labels, build commands, packet text, and bounded preview diagnostics are not rendered in that rail. The repository-side resolver still derives and revalidates those facts, and the Review Set packet still carries the safe repo-relative route Codex needs. The doctor-derived `GET /api/projects/<slug>/authoring-brief` route remains available to repository tooling but is not part of the default annotation UI.

Opening a workspace preview uses a separate tab. The top-level bridge exposes **Inspect**, the same shared Review Set workflow, and **Return to Studio**; it never adds those controls inside the embedded iframe. Full-preview save, note-edit, remove, clear, copy, and return actions travel over the standalone private channel. A connected return focuses the existing Studio session and closes the auxiliary preview instead of navigating that preview into a second empty Studio instance. Studio therefore keeps the temporary Review Set in memory across preview open/close cycles and resynchronizes it when another preview connects. Studio remains the sole Review Set owner, performs every source resolution and revalidation, and sends only bounded item summaries plus the prepared packet back to the connected preview. The preview never receives filesystem access, persistent Review Set storage, or direct source-writing authority.

Optional screenshot annotation uses the Studio document's explicit `getDisplayMedia` permission flow. The isolated preview is denied that capability. Studio registers an approved stream for cleanup as soon as permission resolves, so a later failed selection refresh cannot leave it active; after one approved browser-tab frame it stops all tracks before crop/PNG processing. When the teacher saves an annotation, Studio renders the marker into a PNG and sends only that bounded PNG to the exact-origin `POST /api/inspection/screenshots` route. The route validates PNG signature, byte size, dimensions, project/session/item identities, and a five-file session cap before writing to the ignored `.runtime/studio-review-sets/<session>/` cache. Review Set V2 text contains only safe repo-relative screenshot paths—never pixels, base64, blob URLs, or absolute local paths—and labels screenshot pixels as untrusted course evidence. Cached sessions are eligible for removal during later captures after seven days.
