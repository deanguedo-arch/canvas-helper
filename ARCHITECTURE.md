...
- Durable atomic writes, the cross-process filesystem lock, write-set fingerprints, and crash-recovery journal live in `app/server/lib/course-edit-transaction.ts`.
- Isolated browser postcondition and accessibility checks live in `app/server/lib/course-edit-render-validation.ts`; bounded image decoding and dimension checks live in `app/server/lib/course-edit-image.ts`.
...
One apply request is transactional: preflight and label every draft, re-resolve opaque identities, rebase only when the selected element digest is unchanged, sanitize delta-only operations, acquire a per-course filesystem lock, fingerprint and checkpoint the complete write set, durably journal each phase, update canonical inputs, and run one bounded rebuild if required. Generated checkpoints include builder-owned English resource directories. Builder process groups receive bounded TERM/KILL shutdown so descendants cannot mutate files during recovery. A startup recovery pass rolls an interrupted transaction back before the next mutation; concurrent external drift preserves the journal for manual recovery instead of overwriting it.
...
After filesystem validation, an isolated Playwright page loads the finished learner route with a bounded 30-second navigation allowance and verifies the requested DOM/attribute/style result survived course JavaScript. The larger bound covers the catalog's multi-thousand-element legacy pages without weakening the postcondition. The same pass checks visibility, image alt text, control accessible names, nonempty heading text, and text contrast. Failure restores the checkpoint. Undo first fingerprints the whole post-batch boundary and refuses after any newer work; it never restores merely because a checkpoint exists. Checkpoints, status, locks, and journals live only in ignored `.runtime/studio-edit-*` directories.

Draft persistence is separately versioned from Review Set persistence. Drafts retain their complete server-issued baseline, have no time-based expiry, warn before bounded project eviction, and support strict JSON backup/restore. The React editor and Full Preview bridge use content-editable rich text rather than exposing entities, emit only changed fields, and restrict style keys by semantic tag. Sandboxed visual comparisons retain accessible text captions. Image uploads accept bounded PNG/JPEG/GIF bytes, store content-addressed originals under `projects/resources/<slug>/studio-assets/`, and materialize builder-safe copies under `workspace/assets/custom/studio/`. Rename is a dedicated checkpointed operation over marked HTML title surfaces, project metadata, stored course metadata, and declared runtime string markers.
...
Export freshness is evidence, not a mutable boolean. `scripts/lib/course-editing/export-freshness.ts` fingerprints the canonical workspace together with each produced artifact, and every exporter records that evidence after successful output. Later workspace or artifact drift makes only the matching target stale. Brightspace directory/package, Google Hosted, Apps Script, HTML, SCORM 2004, and SCORM 1.2 are separate evidence targets.
