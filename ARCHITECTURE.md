...
Opening a workspace preview uses a separate tab. The top-level bridge exposes **Inspect**, the same shared Review Set workflow, and **Return to Studio**; it never adds those controls inside the embedded iframe. Full-preview save, note-edit, remove, clear, and copy actions travel over the standalone private channel. Studio remains the sole Review Set owner, performs every source resolution and revalidation, and sends only bounded item summaries plus the prepared packet back to the connected preview. The preview never receives filesystem access or direct source-writing authority.
