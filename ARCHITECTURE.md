...
The Studio Inspector also exposes a doctor-derived Course Build Brief through `GET /api/projects/<slug>/authoring-brief`. The route returns only bounded repo-relative paths, commands, mode, and summarized issues—never source bodies or absolute paths. Review Set re-checks can reveal an opaque saved node and require a fresh selection before confirming that the safe source/rebuild route is unchanged. A subsequent Workspace Verify result is evidence about the workspace command only, not an automatic claim that the learner-facing change is complete. Preview Health keeps at most six bounded runtime/asset diagnostics in browser memory and excludes them from handoffs. Opening a workspace preview uses a separate tab; the bridge adds a top-level-only mini inspector and **Return to Studio** control, never extra controls in the embedded preview. A selection made there enters the same repository-side resolver and Review Set path as an embedded selection.
