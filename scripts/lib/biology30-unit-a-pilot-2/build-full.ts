...
function fullReadingReport(html: string) {
  const $ = loadHtml(html);
...
function estimateWorstCaseState(html: string, fixedVocabularyIds: string[]) {
  const $ = loadHtml(html);
...
function buildRenderedFigureInventory(input: {
  html: string;
