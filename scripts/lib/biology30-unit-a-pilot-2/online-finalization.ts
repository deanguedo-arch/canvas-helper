import { FIRST_USE_REPAIRS } from "./first-use-repair.js";
...
export function buildOnlineFinalizationReport(beforeHtml:string,afterHtml:string,generatedAt:string,estimatedCharacters:number){
  ...
  for(const selector of ["[data-core-zone]","[data-practice-id]","[data-advanced-block-id]","[data-study-task]",".retrieve-block",".evidence-block","[data-textbook-review-route]"]){
    ...
    original.forEach((node,index)=>{
      let expected = before.html(node);
      const repairs = selector === "[data-core-zone]" ? FIRST_USE_REPAIRS.filter(repair => before(node).attr("data-core-zone") === repair.id) : [];
      for (const repair of repairs) {
        assert.ok(expected.includes(repair.before), `First-use baseline passage drift: ${repair.id}`);
        expected = expected.replace(repair.before, repair.after);
      }
      assert.equal(after.html(current[index]), expected, `${selector} changed outside the recorded revision`);
    });
  }
