...
.lessons-nav.is-open .lesson-subnav { display: grid; gap: 4px; }
.sublesson-link {
  display: block;
  padding: 7px 0;
  color: #dbe2d8;
...
@media (max-width: 1100px) {
  ...
  body:not(.sidebar-collapsed) .lessons-nav { display: contents; }
...
function renderShellScript(options: NextStepCourseShellOptions) {
  ...
  const pageIds = ["overview", "lessons", ...lessonIds, ...(options.navItems ?? []).map((item) => item.id)];
...
function showPage(id){
  ...
  if (fallback === "lessons" || lessonIds.includes(fallback)) setLessonsOpen(true);
}
function route(){
