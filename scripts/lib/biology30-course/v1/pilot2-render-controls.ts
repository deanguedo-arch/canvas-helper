...
export function renderTopicPractice(item: RenderPractice, schema: TopicStateSchema, number: number, graph?: GraphWork) {
  ...
  if (item.kind === "multiple-choice") {
    ...
    feedback = item.misconceptionFeedback.map((text, index) => `<p data-pilot2-option-feedback="${index}" hidden>${index === item.correctIndex ? "Correct. " : "Review this choice. "}${topicHtml(index === item.correctIndex ? text.replace(/^Correct\.\s*/i, "") : text)}</p>`).join("");
