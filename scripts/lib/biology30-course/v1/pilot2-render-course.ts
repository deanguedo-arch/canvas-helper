import {LESSON_VIDEO_PLACEMENTS} from './pilot2-video-placement.js';
...
export function renderTopicCourse(input:TopicCourseInputs&{wordData?:BiologyWordPageData},legacySchema?:Biology30SuspendDataSchema) {
 ...
 for(const lesson of contract.topics){
  const placement=LESSON_VIDEO_PLACEMENTS[lesson.id];if(!placement)continue;
  const video=input.sourceVideos?.find(v=>v.videoId===placement.videoId&&v.topics.some(t=>t.id===lesson.id)),part=lesson.parts[placement.part];
  if(!video||!part)throw Error(`Missing woven video source/part for ${lesson.id}`);
  const page=pages.get(lesson.id)!,$topic=load(page.html,null,false),teaching=$topic(`[id="${part.id}"]`).children('.lesson-block').first();
  if(teaching.length!==1)throw Error(`Missing teaching placement ${part.id}`);
  teaching.after(`<section class="lesson-block lesson-source-videos" id="${h(lesson.id)}-source-video-${h(video.videoId)}" data-p2-video-part="${h(part.id)}" tabindex="-1"><p class="section-label">Watch and make sense of it</p><p><strong>Watch for:</strong> ${h(placement.watchFor)}</p><p>Use this video to revisit the explanation above. You can use the diagram and illustrated walkthrough instead.</p>${renderSourceVideo({...video,topics:video.topics.filter(t=>t.id===lesson.id)},`${lesson.id}-source-player-${video.videoId}`)}<p><a href="#${unit}-video-library" data-pilot2-return-route="${unit}-video-library" data-pilot2-return-focus="source-library-${h(video.videoId)}">More videos in the Video Library</a></p></section>`);
