...
const moduleDir = path.resolve(projectPaths.workspaceDir, "content", "WR30_Chapter8_Source_Content_Web_Module");
...
test("world religions option1 chapter 8 wires a local source-content module", async () => {
...
test("world religions option1 chapter 8 ships the normalized content module files", async () => {
  assert.equal(await fileExists(moduleHtmlPath), true, "expected Chapter 8 module index.html");
  assert.equal(await fileExists(moduleCssPath), true, "expected Chapter 8 module styles.css");
