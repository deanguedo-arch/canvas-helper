...
const moduleDir = path.resolve(projectPaths.workspaceDir, "content", "WR30_Chapter5_Source_Content_Web_Module");
...
test("world religions option1 chapter 5 wires a local source-content module", async () => {
  ...
  assert.match(chapter.summary, /Buddhism/i);
});

test("world religions option1 chapter 5 ships the normalized content module files", async () => {
  assert.equal(await fileExists(moduleHtmlPath), true, "expected Chapter 5 module index.html");
  assert.equal(await fileExists(moduleCssPath), true, "expected Chapter 5 module styles.css");
