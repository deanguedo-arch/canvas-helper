...
const moduleDir = path.resolve(projectPaths.workspaceDir, "content", "WR30_Chapter10_Source_Content_Web_Module");
...
test("world religions option1 chapter 10 wires a local source-content module", async () => {
  ...
  assert.match(chapter.summary, /New Millennium/i);
});

test("world religions option1 chapter 10 ships the normalized content module files", async () => {
  assert.equal(await fileExists(moduleHtmlPath), true, "expected Chapter 10 module index.html");
  assert.equal(await fileExists(moduleCssPath), true, "expected Chapter 10 module styles.css");
