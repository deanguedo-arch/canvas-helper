...
const mainPath = path.resolve("projects/mentalwellness10-option2/workspace/main.js");

test("mental wellness option 2 phase 2 lesson rebuild is present in main runtime", async () => {
...
test("mental wellness option 2 phase 2 quiz is added and quiz detail back label is not hardcoded to phase 1", async () => {
  const source = await readFile(mainPath, "utf8");
