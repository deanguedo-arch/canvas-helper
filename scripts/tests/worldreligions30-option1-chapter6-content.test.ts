...
const moduleDir = path.resolve(projectPaths.workspaceDir, "content", "WR30_Chapter6_Source_Content_Web_Module");
...
test("world religions option1 chapter 6 wires a local source-content module", async () => {
  ...
  assert.match(chapter.summary, /Judaism/i);
});

test("world religions option1 chapter 6 ships the normalized content module files", async () => {
  assert.equal(await fileExists(moduleHtmlPath), true, "expected Chapter 6 module index.html");
  assert.equal(await fileExists(moduleCssPath), true, "expected Chapter 6 module styles.css");
