...
const moduleDir = path.resolve(projectPaths.workspaceDir, "content", "WR30_Chapter9_Source_Content_Web_Module");
...
test("world religions option1 chapter 9 wires a local source-content module", async () => {
  ...
  assert.match(chapter.summary, /Sikhism/i);
});

test("world religions option1 chapter 9 ships the normalized content module files", async () => {
  assert.equal(await fileExists(moduleHtmlPath), true, "expected Chapter 9 module index.html");
  assert.equal(await fileExists(moduleCssPath), true, "expected Chapter 9 module styles.css");
