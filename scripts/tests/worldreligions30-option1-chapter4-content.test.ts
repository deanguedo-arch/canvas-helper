...
const moduleDir = path.resolve(projectPaths.workspaceDir, "content", "WR30_Chapter4_Source_Content_Web_Module");
...
test("world religions option1 chapter 4 wires a local source-content module", async () => {
  const dataSource = await readFile(dataPath, "utf8");
  const data = loadCourseData(dataSource);
  const chapter = data?.chapters?.find((entry) => entry.id === "chapter-4");

  assert.ok(chapter);
  assert.equal(chapter.title, "Hinduism");
  assert.equal(chapter.contentPath, "./content/WR30_Chapter4_Source_Content_Web_Module/index.html");
  assert.match(chapter.summary, /Hinduism/i);
});

test("world religions option1 chapter 4 ships the normalized content module files", async () => {
  assert.equal(await fileExists(moduleHtmlPath), true, "expected Chapter 4 module index.html");
  assert.equal(await fileExists(moduleCssPath), true, "expected Chapter 4 module styles.css");
