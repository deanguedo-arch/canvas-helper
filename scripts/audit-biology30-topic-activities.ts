...
import { biology30TopicDesignSha256, type TopicContract } from "./lib/biology30-course/v1/pilot2-contract.js";
...
for (const unit of ["B", "C", "D"]) {
  ...
  const input: ActivityInputs & {contract: TopicContract} = { contract: await load("contract"), state: await load("state-schema"), framing: await load("topic-teaching"), instruction: await load("instruction"),
