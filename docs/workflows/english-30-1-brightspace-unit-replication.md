...
Use this workflow when importing a new English Brightspace unit and converting it into the same polished course-shell pattern proven by the `ela30-1-modern-drama` Streetcar project and refined by the `ela30-1-short-stories`, `ela30-1-shakespeare-othello`, `ela20-1-novel-study-clean`, and `ela20-1-feature-film` builds.

The goal is not to clone one finished course. The goal is to reuse the strongest current shell behavior: the Short Stories / Feature Film navigation and support-surface patterns, the Othello evidence-rich literature tools, the Novel Study reading notebook, and the SCORM-ready autosave / print / media gates.
...
### Current Standard Shell Behaviors

Use the shared shell and recent ELA builds for these non-negotiables:

- Topbar logo remains centered and progress remains pinned right.
- Sidebar collapse mode hides subnav text; expanded mode uses compact grouped navigation.
- Lesson sequence uses grouped horizontal/card lesson groups rather than a plain vertical dump.
- Reading/viewing guides use an evidence notebook when students need to collect reusable notes.
- Text/story/scene/question selectors are dropdown-driven and populate the selected activity below the selector.
- Resources use grouped dropdown browsing with clean student-facing titles.
- Film Room uses a media playlist with one active media stage and organized clips.
- Print/PDF buttons print only the active assignment/page, not the whole course.
- Autosave stores free responses and evidence-bank JSON under stable `canvas-helper:<slug>:responses` keys.
- SCORM export is regenerated after every workspace change and verified at the package level.
...
### Evidence Notebook Pattern

Use this pattern for any reading or viewing process where students need a reusable collection of proof:

- setup fields for title/text/film, pass or stage, and current focus
- full-width baseline prompts stacked vertically, not squeezed side by side
- one evidence-entry form with fields for location, technique/type, moment/quotation, context, author/director choice, effect, and why it matters
- persistent evidence bank with filter, edit/delete, and `Mark strongest`
- synthesis prompts that help transfer saved notes into questions, paragraphs, or essays
- hidden JSON response field so the bank rides on the shared autosave/SCORM bridge
- `Print Reading Portfolio`, `Print Evidence Portfolio`, or equivalent scoped print button

Do not make the guide just four static text boxes if the unit expects students to collect evidence across a full text or film.
...
- `Print / PDF` buttons should clone only the intended assignment/page; hidden pages and inactive question sets must stay hidden in the print clone
- Studio/Course Showcase preview should show the course beside at least one trusted reference build before final packaging when shell parity matters
...
- if the course has an evidence notebook, save a card, reload, and confirm it restores without duplicate stale drafts
- click one print/PDF button on a question page or guide and confirm it does not print the whole course
...
- evidence notebooks are used for long-form reading/viewing evidence collection
...
- print/PDF is scoped to the current assignment/page
