...
- Opt-in SCORM action reporting: keep aggregates ungraded, retain stable interaction rows and counts across launches, test failed-write retry and hidden/idle timing, and run `node_modules/.bin/tsx scripts/tests/social-option2-scorm.mts` when Social contracts change. Verify actual Brightspace report displays separately from simulated API assertions.
